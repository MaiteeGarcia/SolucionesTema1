...
```{r}
commonAnimals <- intersect(df_1, df_2)
...
a <- setdiff(rownames(df_1), rownames(df_2))
a

"Animales en Animals2 pero no en mammals"
b <- setdiff(rownames(df_2), rownames(df_1))
b
```
...
```{r}
#Estan escritos de formas distintas
...
#c
length(libReturn$results) #tengo instaladas 651 librerias
```
...
lm(formula = VitC ~ HeadWt, data = cabbages, subset = which(Cult ==
"c39"))
lm(formula = VitC ~ HeadWt, data = cabbages, subset = which(Cult ==
"c52"))
